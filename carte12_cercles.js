...
var maj_pays = function (json, annee, maptype, dispo) {
	<!-- joint les données (data) dans l ordre du code iso2, qui sert de clé -->
	var pays  = cartogroupe.selectAll("path")
		.data(json.features, function(d) { return d.properties[colonne_code];});
	pays.transition()
...
var maj_carte = function (annee, maptype, dispo) {
	...
	d3.csv(path_csv, function (data) {
		...
		d3.json(path_json, function(json) {
			...
			maj_pays(json, annee, maptype, dispo);
...
dessin_carte(annee, maptype, dispo);
...
d3.selectAll(".rmt")
	.on("click", function() {
		...
		<!-- redessine la carte -->
   		...
   		<!-- met à jour la description du type de carte -->
		show_hide_multiple(["desc_" + maptype], ["desc_ana", "desc_cp"]);
		
		<!-- met à jour la légende -->
		show_hide_multiple(["leg_" + maptype], ["leg_ana", "leg_cp"]);
